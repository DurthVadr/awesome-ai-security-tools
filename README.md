...
- **[SpendShield](https://github.com/felixpg13-glitch/spendshield)** 🟢 — Python policy & authorization layer for AI-agent spending decisions: deterministic ALLOW / APPROVAL / DENY against budgets, per-transaction caps, and merchant allow/block lists, with a hash-chained audit trail of every decision; ships as a library and an MCP server and is channel-agnostic (x402, Stripe, wallets). — **note:** the MCP server does not itself enforce tool-level separation — hosts should expose only decision tools (spend_authorize / spend_status / spend_audit) to untrusted agents and keep management tools (spend_approve / spend_reset / policy_*) host-side. A signed single-use grant executor exists as an explicitly labeled experimental prototype, separate from the default authorize() flow; the audit hash chain detects partial tampering but is not keyed or externally anchored. *(★ 2 · updated 2026-09-06)*
